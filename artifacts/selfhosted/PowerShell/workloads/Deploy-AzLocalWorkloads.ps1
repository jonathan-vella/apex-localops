...
.DESCRIPTION
    ...
    action is a cloud/ARM call, so nothing needs to run on the cluster host. Imports
    ...
    This script is HUMAN-INVOKED (directly, or via scripts/deploy-workloads-selfhosted.sh).
...
if ($Stage -in @('ws2025', 'sql', 'avd-host', 'all') -and -not $WhatIfPreference) {
    $pw = Resolve-AdminPassword    # from LOCALSELF_ADMIN_PASSWORD env var; never logged
}
