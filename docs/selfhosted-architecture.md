...
```mermaid
flowchart TB
    subgraph HOST["apex-host (Hyper-V) · internal switch ApexLocal-Internal + host NAT 192.168.1.0/24"]
        DC["apexlocal-dc<br/>AD DS forest apexlocal.local<br/>DNS + NTP authority · 192.168.1.254"]
        N1["apexlocal-n1 · 192.168.1.11<br/>96 GB · TPM/SecureBoot<br/>+ StorageA/StorageB NICs"]
        N2["apexlocal-n2 · 192.168.1.12<br/>96 GB · TPM/SecureBoot"]
        N3["apexlocal-n3 · 192.168.1.13<br/>96 GB · TPM/SecureBoot"]
    end
...
sequenceDiagram
    ...
    Host->>Host: Phase 2 — internal switch + NAT, then WAIT for ISOs
...
a nested Hyper‑V VM. So both nested base images — the Windows Server DC base and
the Azure Local node base — are built from **ISOs** via DISM
([`Convert-ApexIsoToVhdx`](../artifacts/selfhosted/PowerShell/ApexLocalOps/ApexLocalOps.psm1)).
The two *Azure* VMs (host + jumpbox) still boot from a normal WS2025 marketplace
image, which is fine for real Azure VMs.
...
- **Fabric networking** (`New-ApexHostSwitch` + node storage NICs) — intent‑based
  management/compute + storage adapters, converged on the internal switch.
