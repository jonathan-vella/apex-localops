...
Azure Local cluster plus a domain controller inside a single Azure host VM, with a
separate small jumpbox for ISO staging.

## Default footprint (3-node)

| Resource | SKU / size | Notes |
|---|---|---|
| Cluster host `apex-host` | `Standard_E64s_v6` (64 vCPU / 512 GB) | hosts 3 × 96 GB nodes + the DC |
...
| Log Analytics | pergb2018 | host + jumpbox telemetry, build logs |

**Nested VMs (inside `apex-host`):** 3 × node (96 GB / 16 vCPU each) + 1 × DC
(4 GB / 4 vCPU). Total committed nested RAM ≈ 292 GB, comfortably under the host's
512 GB, leaving headroom for the host OS, the storage pool cache, and Arc agents.
