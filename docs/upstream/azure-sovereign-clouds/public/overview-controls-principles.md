...
Azure Key Vault Managed HSM External Key Management (EKM) lets you protect resources with encryption keys that are physically held outside of Microsoft premises in a customer-operated HSM. This capability is aimed at highly regulated workloads where encryption must use keys stored at a customer-controlled facility. EKM ensures that wrap and unwrap operations occur exclusively on customer-controlled hardware, but it has an impact on the SLAs of dependent services. The decision of when to use External Key Management must be based on several factors that are explained in the [Sovereign Concepts - Key Management](../key-controls.md) chapter.
...
By default, most services in Azure use in-transit encryption. Most services use a TLS certificate generated by the service upon provisioning and signed by a public certificate authority. For more information on in-transit encryption for services, see the [Data Security and encryption page](/azure/security/fundamentals/encryption-overview#encryption-of-data-in-transit).
...
- Assess confidential compute applicability by using [Confidential Computing](confidential-computing.md).
