...
Self-contained, clean-room automation — a nested Azure Local cluster (**Self-hosted**) or an edge device (**SFF**), with no Arc Jumpstart dependency.
...
[![IaC: Bicep](https://img.shields.io/badge/IaC-Bicep-1BA1E2)](infra/bicep/azlocal-selfhosted/main.bicep)
...
> and not every profile has been validated end to end. The **Self-hosted** profile is the
> primary path; **SFF** and **AKS on bare metal** are **preview** and still evolving. Use it for
> evaluation only, and please
...
| Profile | What it builds | Est. cost (40-h week) | Get started |
| --- | --- | --- | --- |
| **Self-hosted** | A nested 3-node Azure Local cluster plus a domain controller and router, built clean-room from two operator-staged ISOs — no prebaked Jumpstart VHDs, no `Azure.Arc.Jumpstart.*` modules | ~$563 | [Self-hosted overview →](docs/selfhosted/overview.md) |
| **Small Form Factor (SFF)** | A lighter single host that builds the SFF Maintenance OS (ROE) test VM — the edge analogue at roughly a quarter of the cost | ~$151 | [SFF overview →](docs/sff/overview.md) |
...
| --- | --- | --- |
| Self-hosted | ~$6.94/h running, ~$563 per 40-h week | [Self-hosted sizing and cost](docs/selfhosted/sizing.md) |
| SFF | ~$1.68/h running, ~$151 per 40-h week | [SFF sizing and cost](docs/sff/sizing.md) |
...
| --- | --- | --- |
| **Self-hosted** | Primary | Clean-room build; functional and being validated across regions and Azure Local builds. Pin `githubBranch` to a release tag for reproducible deploys. |
...
apex-localops began as a packaging of the **Arc Jumpstart LocalBox** sandbox from
[`microsoft/azure_arc`](https://github.com/microsoft/azure_arc) (`azure_jumpstart_localbox`,
vendored from commit `027b9554b2534af190271bd7443d8556da745d3e`). The LocalBox profile was
**retired in v2.0.0**
([last release: v1.3.0](https://github.com/jonathan-vella/apex-localops/releases/tag/v1.3.0)),
but the current **Self-hosted** profile was derived from that work, so this repository remains a
derivative distributed under the **same license as upstream — Creative Commons Attribution 4.0
International (CC BY 4.0)**; see [LICENSE](LICENSE) and the credit and list of changes in
[ATTRIBUTION.md](ATTRIBUTION.md). Azure Local OS images, PowerShell, and other Microsoft
components are downloaded from Microsoft at build time and remain subject to their own license
terms.
