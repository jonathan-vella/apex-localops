...
@description('Name of an existing logical network on the cluster (for example: apexlocal-InfraLNET).')
param hciLogicalNetworkName string

@description('Name of the custom location for the Azure Local instance (for example: apexlocal-cl).')
param customLocationName string

@description('Optional static private IP for the VM NIC (within the logical network subnet). Empty = auto-allocate from the IP pool.')
...
resource nic 'Microsoft.AzureStackHCI/networkInterfaces@2024-01-01' = {
  ...
  properties: {
    ipConfigurations: [
      {
        name: 'ipconfig1'
        // Add a static privateIPAddress only when supplied; otherwise auto-allocate from the pool.
        properties: union(
          {
            subnet: {
              id: logicalNetworkId
          ...
          empty(privateIPAddress) ? {} : { privateIPAddress: privateIPAddress }
        )
      }
    ]
  }
